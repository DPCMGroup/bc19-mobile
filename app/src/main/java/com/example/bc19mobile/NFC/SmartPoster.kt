 ...
 */
class SmartPoster(uri: UriRecord?, title: TextRecord?, action: com.example.bc19mobile.NFC.SmartPoster.RecommendedAction?, type: String) : ParsedNdefRecord {
    /**
     ...
     */
    private val mAction: com.example.bc19mobile.NFC.SmartPoster.RecommendedAction
        ...
        companion object {
            ...
            init {
                ...
                for (action in values()) {
                    //com.example.bc19mobile.builder.put(com.example.bc19mobile.action.getByte(), com.example.bc19mobile.action)
    ...
    companion object {
        ...
        fun parse(recordsRaw: Array<NdefRecord?>?): com.example.bc19mobile.NFC.SmartPoster {
            return try {
                ...
                val title: TextRecord? = com.example.bc19mobile.NFC.SmartPoster.Companion.getFirstIfExists(records, TextRecord::class.java)
                val action: RecommendedAction? = com.example.bc19mobile.NFC.SmartPoster.Companion.parseRecommendedAction(recordsRaw)
                val type: String? = com.example.bc19mobile.NFC.SmartPoster.Companion.parseType(recordsRaw)
                com.example.bc19mobile.NFC.SmartPoster(uri, title, action, type.toString())
        ...
        fun isPoster(record: NdefRecord?): Boolean {
            return try {
                //com.example.bc19mobile.SmartPoster.Companion.parse(record)
                parse(record)
                true
        ...
        private fun parseRecommendedAction(records: Array<NdefRecord?>?): RecommendedAction? {
            val record: NdefRecord = com.example.bc19mobile.NFC.SmartPoster.Companion.getByType(com.example.bc19mobile.NFC.SmartPoster.Companion.ACTION_RECORD_TYPE, records)
                    ?: return com.example.bc19mobile.NFC.SmartPoster.RecommendedAction.UNKNOWN
            val action = record.payload[0]
            return if (com.example.bc19mobile.NFC.SmartPoster.RecommendedAction.Companion.LOOKUP?.containsKey(action) == true) {
                com.example.bc19mobile.NFC.SmartPoster.RecommendedAction.Companion.LOOKUP!!.get(action)
            } else com.example.bc19mobile.NFC.SmartPoster.RecommendedAction.UNKNOWN
        }
        
        private val TYPE_TYPE = byteArrayOf('t'.toByte())
        private fun parseType(records: Array<NdefRecord?>?): String? {
            val type: NdefRecord = com.example.bc19mobile.NFC.SmartPoster.Companion.getByType(com.example.bc19mobile.NFC.SmartPoster.Companion.TYPE_TYPE, records)
                    ?: return null
